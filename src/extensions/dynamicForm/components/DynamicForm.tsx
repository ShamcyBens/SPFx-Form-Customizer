...
const DynamicForm: React.FC<IDynamicFormProps> = ({ context, displayMode, onSave, onClose }) => {
  ...
  useEffect(() => {
    const loadTemplate = async (): Promise<void> => {
      try {
        ...
        const listUrl = `${context.pageContext.web.absoluteUrl}/_api/web/lists/getbytitle('YourListName')/items(${itemId})`;
        ...
        const templateUrl = `${context.pageContext.web.absoluteUrl}/_api/web/lists/getbytitle('Templates')/items(${templateId})`;
  ...
  const updateField = (id: number, property: string, value: any): void => {
  ...
  const saveTemplate = async (): Promise<void> => {
    ...
    const listUrl = `${context.pageContext.web.absoluteUrl}/_api/web/lists/getbytitle('Templates')/items`;
    await fetch(listUrl, {
      method: 'POST',
      headers: {
        'Accept': 'application/json;odata=verbose',
  ...
  const submitForm = async (event: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    const listUrl = `${context.pageContext.web.absoluteUrl}/_api/web/lists/getbytitle('YourListName')/items`;
    await fetch(listUrl, {
      method: 'POST',
      headers: {
        'Accept': 'application/json;odata=verbose',
